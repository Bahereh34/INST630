...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    // Step 1: Filter restaurants by price range
    const cheapRestaurants = restaurants.filter((restaurant) => {
        return restaurant.priceRange === "$" || restaurant.priceRange === "$$";
    });

    // Step 2: Display the filtered restaurants
    cheapRestaurants.forEach((restaurant) => {
        filteredList.innerHTML += `
            <div>
                <strong>${restaurant.name}</strong> - ${restaurant.cuisine} (${restaurant.priceRange})
            </div>
        `;
    });
...
if (bestRestaurant) {
    foundItem.innerHTML = `
        <div>
            <strong>${bestRestaurant.name}</strong> - 
            ${bestRestaurant.cuisine} | 
            Rating: ${bestRestaurant.rating}
        </div>
    `;
} else {
    foundItem.innerHTML = `<div>No restaurant found with rating 4.8</div>`;
}
